...
class AuthService {
    async register(userRegisterInfo: BodyRegister) {
        ...
        if (res) throw ApiError.BadRequest("User already exist");
    ...
    async login(userLoginInfo: BodyLogin) {
        ...
        if (!res) {
            throw ApiError.BadRequest("Login or password error");
        }
        const checkPassword = await bcrypt.compare(userLoginInfo.password, res.password)
        if (!checkPassword) throw ApiError.BadRequest("Login or password error");
